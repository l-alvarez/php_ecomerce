...
function sql_dump_result($result) {
    ...
    $head = '<tr id="ctabla"><th>' . LABEL_LEFT_TIME . '</th><th>' . LABEL_PRICE . '</th><th>' . LABEL_NOM . '</th><th>' . LABEL_FOTO . '</th></tr>';
    ...
    while ($temp = mysql_fetch_assoc($result)) {
        if ($temp['estat'] == 0) {
            ...
            $line .= '<tr><td> <img src="http://gifcountdown.com/europe-madrid/' . $mili . '/fdfdfd/000000/000000/333333/000000/true/counter.gif" WIDTH=170 HEIGHT=100> ' .
                    '</td><td>' . ( $temp['preu_actual'] + $temp['increment'] ) . '€</td>' .
                    '<td><a href="../Controllers/Command.php?controller=SubastaController&action=show&id=' . $temp['id_subhasta'] . '">' . $producte['nom'] . '</a></td>' .
